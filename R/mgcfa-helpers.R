...
.mgcfa_is_correlation_matrix <- function(m, tol = 1e-8) {
  if (!is.matrix(m) || nrow(m) != ncol(m)) {
    return(FALSE)
  }
  if (any(abs(m - t(m)) > tol, na.rm = TRUE)) {
    return(FALSE)
  }
  d <- diag(m)
  if (any(abs(d - 1) > 1e-6, na.rm = TRUE)) {
    return(FALSE)
  }
  all(abs(m) <= (1 + 1e-6), na.rm = TRUE)
}

.mgcfa_resolve_ordered_vars <- function(ordered, data, group, variables = NULL) {
  if (is.null(ordered) || identical(ordered, FALSE)) {
    return(character())
  }
  if (identical(ordered, TRUE)) {
    ...
    vars <- as.character(ordered)
  }
  vars <- unique(vars[nzchar(vars)])
  if (length(vars) == 0L) {
    stop("`ordered` did not resolve to any variables.", call. = FALSE)
  }
  if (!all(vars %in% names(data))) {
    miss <- setdiff(vars, names(data))
    stop("Ordered variables not found in data: ", paste(miss, collapse = ", "), call. = FALSE)
  }
  vars
}

.mgcfa_handle_validation <- function(msg, action = c("warn", "error", "none")) {
...
.mgcfa_validate_summary_matrices <- function(sample_cov, sample_nobs, sample_mean = NULL, action = c("warn", "error", "none")) {
  action <- match.arg(action)
  if (identical(action, "none")) {
    return(invisible(TRUE))
  }
  if (!is.list(sample_cov) || length(sample_cov) < 1L) {
    return(invisible(TRUE))
  }
  nobs <- as.numeric(unlist(sample_nobs))
  if (length(nobs) == 1L) {
    nobs <- rep(nobs, length(sample_cov))
  }
  for (i in seq_along(sample_cov)) {
    m <- sample_cov[[i]]
    lbl <- paste0("group ", i)
    if (any(abs(m - t(m)) > 1e-6, na.rm = TRUE)) {
      .mgcfa_handle_validation(sprintf("Summary matrix for %s is not symmetric within tolerance.", lbl), action = action)
    }
    eig <- tryCatch(eigen(m, symmetric = TRUE, only.values = TRUE)$values, error = function(e) numeric())
    if (length(eig) > 0L) {
      if (min(eig, na.rm = TRUE) <= 0) {
        .mgcfa_handle_validation(sprintf("Summary matrix for %s is not positive definite.", lbl), action = action)
      } else if (max(eig, na.rm = TRUE) / min(eig, na.rm = TRUE) > 1e8) {
        .mgcfa_handle_validation(sprintf("Summary matrix for %s is near-singular (high condition number).", lbl), action = action)
      }
    }
    if (length(nobs) >= i && is.finite(nobs[[i]]) && nobs[[i]] <= ncol(m)) {
      .mgcfa_handle_validation(
        sprintf("Sample size for %s (%s) is small relative to variable count (%s).", lbl, nobs[[i]], ncol(m)),
        action = action
      )
    }
  }
  if (!is.null(sample_mean)) {
    ...
    if (length(sample_mean) != length(sample_cov)) {
      .mgcfa_handle_validation("`sample_mean` length does not match number of groups.", action = action)
    }
  }
  invisible(TRUE)
}
...
.mgcfa_step_group_equal <- function(
  ...
  if (identical(step, "scalar")) {
    if (isTRUE(use_thresholds)) {
      return(c("loadings", "thresholds"))
    }
    return(c("loadings", "intercepts"))
  }
  if (identical(step, "strict")) {
    if (isTRUE(use_thresholds)) {
      return(c("loadings", "thresholds", "residuals"))
    }
    return(c("loadings", "intercepts", "residuals"))
  }

  base <- if (identical(anchor, "strict")) {
    if (isTRUE(use_thresholds)) c("loadings", "thresholds", "residuals") else c("loadings", "intercepts", "residuals")
  } else {
    if (isTRUE(use_thresholds)) c("loadings", "thresholds") else c("loadings", "intercepts")
  }
...
.mgcfa_nested_lrt_compare <- function(previous_fit, candidate_fit) {
  ...
  lrt <- tryCatch(
    suppressWarnings(lavaan::lavTestLRT(previous_fit, candidate_fit)),
    error = function(e) NULL
  )
  if (is.null(lrt)) {
    return(fallback())
...
.mgcfa_eval_partial_candidate <- function(
  added_terms,
  step,
  group_equal,
  added_constraints,
  ...
  list(
    row = row,
    fit = if (is_ok) fit_or_error else NULL,
    partial = partial_now,
    added = added_terms,
    stage_acceptable = isTRUE(stage_acceptable_i)
  )
...
.mgcfa_search_partial_step <- function(
  ...
  for (k in 0:max_free) {
    ...
    remaining_slots <- max_models - (iter_id + 1L)
    if (remaining_slots <= 0L) {
      truncated <- TRUE
      break
    }
    if (length(combos_k) > remaining_slots) {
      combos_k <- combos_k[seq_len(remaining_slots)]
      truncated <- TRUE
    }

    pass_found_k <- FALSE
    if (isTRUE(parallel_enabled) && length(combos_k) > 1L) {
      evals <- parallel::mclapply(
        combos_k,
        function(added_terms) {
          .mgcfa_eval_partial_candidate(
      ...
      for (ev in evals) {
        iter_id <- iter_id + 1L
        row_i <- ev$row
        row_i$iteration <- iter_id
        candidate_rows[[length(candidate_rows) + 1L]] <- row_i
        candidate_fits[[length(candidate_fits) + 1L]] <- ev$fit
        candidate_partials[[length(candidate_partials) + 1L]] <- ev$partial
        candidate_added[[length(candidate_added) + 1L]] <- ev$added
        if (isTRUE(ev$stage_acceptable)) {
          pass_found_k <- TRUE
        }
      }
    } else {
      for (added_terms in combos_k) {
        ev <- .mgcfa_eval_partial_candidate(
          added_terms = added_terms,
          step = step,
          group_equal = group_equal,
          added_constraints = added_constraints,
